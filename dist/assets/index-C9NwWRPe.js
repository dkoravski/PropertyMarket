(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))o(e);new MutationObserver(e=>{for(const a of e)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&o(n)}).observe(document,{childList:!0,subtree:!0});function s(e){const a={};return e.integrity&&(a.integrity=e.integrity),e.referrerPolicy&&(a.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?a.credentials="include":e.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(e){if(e.ep)return;e.ep=!0;const a=s(e);fetch(e.href,a)}})();function l(t="/"){return`
    <header class="bg-white border-bottom shadow-sm sticky-top">
      <nav class="navbar navbar-expand-lg container py-2">
        ...
        <li class="nav-item">
          <a class="nav-link ${t===o?"active":""}" href="#${o}">${e}</a>
        </li>
      `).join("")}</ul>
        </div>
      </nav>
  ...
  `}function d(){return`
    ...
    <section aria-label="Препоръчани обяви">
      ...
      <div class="row g-3">
        ${c("Модерен апартамент","София","€145,000")}
        ${c("Семейна къща","Пловдив","€220,000")}
        ${c("Уютно студио","Варна","€95,000")}
      </div>
    </section>
  `}function c(t,r,s){return`
    <article class="col-12 col-md-6 col-lg-4">
      <div class="card h-100 shadow-sm border-0">
        <div class="card-body">
          <h3 class="h5 card-title mb-2">${t}</h3>
          <p class="mb-1 text-muted">${r}</p>
  ...
  `}const P={"/":{pageFactory:d},"/about":{pageFactory:p},"/listings":{pageFactory:u},"/property":{pageFactory:b},"/login":{pageFactory:m},"/register":{pageFactory:h},"/create-property":{pageFactory:g,requiresAuth:!0},"/edit-property":{pageFactory:f,requiresAuth:!0},"/profile":{pageFactory:y,requiresAuth:!0},"/favorites":{pageFactory:v,requiresAuth:!0},"/admin":{pageFactory:w,requiresAdmin:!0}};function x(t,r={}){const s=r.getAuthState||F,o=()=>{const e=A(),a=P[e],n=$(s());if(!a){t({path:e,pageContent:L()});return}if(a.requiresAdmin&&n.role!=="admin"){t({path:e,pageContent:M()});return}if(a.requiresAuth&&!n.isAuthenticated){t({path:e,pageContent:q()});return}t({path:e,pageContent:a.pageFactory()})};window.addEventListener("hashchange",o),o()}function A(){return window.location.hash.replace("#","")||"/"}function F(){const t=localStorage.getItem("pm_is_authenticated")==="true",r=localStorage.getItem("pm_user_role");return{isAuthenticated:t,role:r}}function $(t){if(!t)return{isAuthenticated:!1,role:"guest"};const r=!!t.isAuthenticated,s=(t.role||(r?"user":"guest")).toLowerCase();return{isAuthenticated:r,role:s}}function L(){return`
    <section class="rounded-4 p-4 p-md-5 bg-light border">
  ...
  `}function S(t,r){t.innerHTML=r}function O(){const t=document.getElementById("app");if(!t)throw new Error("Основният контейнер на приложението не е намерен.");x(({path:r,pageContent:s})=>{S(t,`${l(r)}
      <main class="container py-4">${s}</main>
      ${i()}`)})}O();
